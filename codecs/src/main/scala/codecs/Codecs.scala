...
trait EncoderInstances:
  ...
  given stringEncoder: Encoder[String] =
    Encoder.fromFunction(n=> Json.Str(String(n)))// TODO Implement the `Encoder[String]` given instance
...
trait DecoderInstances:
  ...
  given intDecoder : Decoder[Int] =
    Decoder.fromPartialFunction { case Json.Num(x:BigDecimal ) if x.isValidInt => x.toInt }
  /** A decoder for `String` values */
  // TODO Define a given instance of type `Decoder[String]`
  given stringDecoder : Decoder[String] =
    Decoder.fromPartialFunction { case Json.Str(x:String ) => x }
  /** A decoder for `Boolean` values */
  // TODO Define a given instance of type `Decoder[Boolean]`
  given booleanDecoder : Decoder[Boolean] =
    Decoder.fromPartialFunction { case Json.Bool(x:Boolean ) => x }
  /**
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
     Decoder.fromFunction { x =>
      x match {
        case Json.Obj(x) => x.get(name).flatMap(x => decoder.decode(x))
        case _ => None
      }
    }
...
trait ContactsCodecs:
  ...
  given Decoder[Contacts]=
    Decoder.field[List[Person]]("people").transform[Contacts](contact => Contacts(contact))
