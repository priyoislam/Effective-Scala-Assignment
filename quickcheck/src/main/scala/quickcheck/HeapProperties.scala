...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val meldingSmallHeaps: (String, Prop) =
    ...
    forAll { (x: Int, y: Int) =>
      ...
      val meldedHeap: List[Node] = meld(insert(minVal, insert(minVal, empty)), insert(maxVal, insert(maxVal, empty)))
  ...
  val meldingHeaps: (String, Prop) =
    def check(
      ...
      if isEmpty(melded) then isEmpty(heap1) && isEmpty(heap2)
      else
        (isEmpty(heap1), isEmpty(heap2)) match
          ...
          case (false, false) =>
            ...
            else check(heap1, deleteMin(heap2), deleteMin(melded))
